...
from utils.Constant import DOOR_OPERATION_TIME, TIME_PER_BOARD, TIME_PER_DEPARTURE

def bus_process(env, bus: Bus, stops: dict[str, Stop], metrics):

    if bus.start_time > 0:
        yield env.timeout(bus.start_time)
    ...
    for i, (stop_id, next_stop_travel_time) in enumerate(bus.stops):
        ...
        if next_stop_travel_time is not None:
            yield env.timeout(next_stop_travel_time / 4)
